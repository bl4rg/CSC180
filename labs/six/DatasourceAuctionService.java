...
public class DatasourceAuctionService implements AuctionService {
	...
	public DatasourceAuctionService() throws IOException {
		items = new FileBasedDatasource("auctions.dat");
	}
	
	public DatasourceAuctionService(Datasource items) {
		this.items = items;
	}
	
	public DatasourceAuctionService(RecordReader rr) {
		while ( rr.hasNext() ) {
			Auction a = rr.next();
			try {
				items.update(a);
	...
	public Auction retreive(Long id) {
		...
		try {
			a = items.findById(id);
			if ( a == null ) {
				throw new ObjectNotFoundException("Auction #" + id + " does not exist");
			}
			return a;
		} catch (IOException e) {
			throw new RuntimeException(e);
	...
	public Auction update(Long id, Auction auction) {
		if ( !auction.getId().equals(id) ) throw new IdMismatchException("Auction #" + auction.getId() + " does not match passed id " + id);
